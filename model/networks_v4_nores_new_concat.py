...
class UnderNet(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, center_num=3, norm='batch', activation='PReLU', drop_rate=0, gpu_ids=[], weight=1.0):
        ...
        # TODO: Decoder_2
        self.up4_2 = Upblock(ngf * 8, ngf * 8)
        self.de4_2 = _BasicBlock(ngf * 16, ngf * 8, norm_layer, nonlinearity, use_bias)
        self.up3_2 = Upblock(ngf * 8, ngf * 4)
        self.de3_2 = _BasicBlock(ngf * 8, ngf * 4, norm_layer, nonlinearity, use_bias)
        self.up2_2 = Upblock(ngf * 4, ngf * 2)
        self.de2_2 = _BasicBlock(ngf * 4, ngf * 2, norm_layer, nonlinearity, use_bias)
        self.up1_2 = Upblock(ngf * 2, ngf)
        self.de1_2 = _BasicBlock(ngf * 2, ngf, norm_layer, nonlinearity, use_bias)

        self.out = _OutputBlock(ngf, output_nc, 7, use_bias)
        self.out_2 = _OutputBlock(ngf, output_nc, 7, use_bias)
    ...
    def forward(self, x, fea_vgg):
        # level 1
        add_fea_1 = self.fea1(fea_vgg[0]) # torch.Size([8, 32, 224, 224])
        conv1 = self.conv1(torch.cat([self.head(x), add_fea_1], dim=1))

        # level 2
        add_fea_2 = self.fea2(fea_vgg[1]) # torch.Size([8, 64, 112, 112])
        conv2_1 = self.down1(conv1)
        conv2 = self.conv2(torch.cat([conv2_1, add_fea_2], dim=1))

        # level 3
        add_fea_3 = self.fea3(fea_vgg[2]) # torch.Size([8, 128, 56, 56])

        conv3_2 = self.down2(conv2)
        conv3 = self.conv3(torch.cat([conv3_2, add_fea_3], dim=1))

        # level 4
        add_fea_4 = self.fea4(fea_vgg[3]) # torch.Size([8, 256, 28, 28])
