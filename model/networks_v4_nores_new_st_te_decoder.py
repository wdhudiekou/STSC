...
class UnderNet(nn.Module):
    ...
    def forward(self, x, fea_vgg):
        ...
        x1 = self.activation(fea_vgg[0]) # torch.Size([8, 64, 224, 224])
        x2 = self.activation(fea_vgg[1]) # torch.Size([8, 128, 112, 112])
        x3 = self.activation(fea_vgg[2]) # torch.Size([8, 256, 56, 56])
        x4 = self.activation(fea_vgg[3]) # torch.Size([8, 512, 28, 28])
        ...
        # TODO: Feature Dominative subnetwork
        ...
        # level 1
        x_in = self.head(x)
        conv1 = self.conv1(x_in) # torch.Size([8, 32, 224, 224])

        # level 2
        conv2_1 = self.down1(conv1)
        conv2 = self.conv2(conv2_1) # torch.Size([8, 64, 112, 112])

        # level 3
        conv3_2 = self.down2(conv2)
        conv3 = self.conv3(conv3_2) # torch.Size([8, 128, 56, 56])

        # level 4
        conv4_3 = self.down3(conv3)
        conv4 = self.conv4(conv4_3) # torch.Size([8, 256, 28, 28])
        ...
        # TODO: Decoder
        # level 4
        deco4 = self.de4(torch.cat([self.up4(center), self.weight * conv4, add_fea_4], dim=1)) # torch.Size([8, 256, 28, 28])

        # level 3
        deco3 = self.de3(torch.cat([self.up3(deco4), 0.8 * self.weight * conv3, add_fea_3], dim=1)) # torch.Size([8, 128, 56, 56])

        # level 2
        deco2 = self.de2(torch.cat([self.up2(deco3), 0.4 * self.weight * conv2, add_fea_2], dim=1)) # torch.Size([8, 64, 112, 112])

        # level 1
        deco1 = self.de1(torch.cat([self.up1(deco2), 0.2 * self.weight * conv1, add_fea_1], dim=1)) # torch.Size([8, 32, 224, 224])
...
if __name__ == '__main__':
    # net = TwoStage().cuda()
    # a = torch.randn(16, 3, 128, 128).cuda()
    # b = torch.randn(16, 3, 128, 128).cuda()
    # c = net([a, b])
    # print(c.shape)
    ...
    # b = torch.randn(1, 3, 640, 480).cuda()
    print(params_count(net) / (1000 ** 2))
    # print(params_count(c) / (1000 ** 2))
    ...
    # out_w = h // width  # 8
    # out_h = w // height  # 4
    # mm = a.view(b, c, out_w, width, out_h, height)
    # mm_1 = mm.permute(0, 1, 2, 4, 3, 5).contiguous().view(b, out_h*out_w, c*height*width)
    # print(mm.shape)
    # print(mm_1.shape)
